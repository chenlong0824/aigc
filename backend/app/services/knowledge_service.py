...
def init_knowledge_base():
    ...
    try:
        # 尝试获取已存在的集合
        collection = client.get_collection("xianyang_travel")
    except Exception:
        # 如果不存在则创建
        collection = client.create_collection("xianyang_travel")
...
用户问题：{user_message}"""
        ...
        return f"智能客服暂时不可用（模型加载中），请稍后重试。您也可以拨打客服热线 029-12345 进行咨询。错误信息：{str(e)[:100]}"
